...
// ==================================================
// The single-threaded writer from upstream leveldb
...
SimpleWriter::SimpleWriter(WritableFile* dest) : dest_(dest), dest_length_(0), block_offset_(0) {
  InitTypeCrc(type_crc_);
}

SimpleWriter::SimpleWriter(WritableFile* dest, uint64_t dest_length)
...
Status SimpleWriter::AddRecord(const Slice& slice) {
  const char* ptr = slice.data();
  size_t left = slice.size();
...
Status SimpleWriter::EmitPhysicalRecord(RecordType t, const char* ptr,
                                  size_t length) {
